...
class ModSimpleFileUploaderHelperv13{	
	...
	function getUploadForm(
			...
			// Get settings:
			$upload_maxsize = $params->get( 'upload_maxsize', '15728640' );
			...
			$database_file_path_prefix = "/var/www/html/joomla/imperialinput/";
			$database_file_output_suffix = ".zip";
			$file_uploaded = false;
			...
			$new_job->username = $usr_name;
			//echo("username:");
			//echo $usr_name;
			$new_job->time_submitted = date('Y-m-d H:i:s');
			...
			//DATABASE
			$new_job->user_email = $usr_email;
			$new_job->time_started = NULL;
			$new_job->time_ended = NULL;
			$new_job->output_path = NULL;
				if ((strlen($_FILES["uploadedfile$mid"]["name"][$cnt]) > 0) && ($upload_users === "true")) {
					...
					if (($filetypeok) && ($_FILES["uploadedfile$mid"]["size"][$cnt] < $upload_maxsize)) {
						...
						if ($_FILES["uploadedfile$mid"]["error"][$cnt] > 0) {
							...
							//DATABASE
							$orig_filename = $_FILES["uploadedfile$mid"]["name"][$cnt];
							//strip ".nii.gz"
							$orig_filename = str_replace(".nii.gz", "", $orig_filename);
							$new_job->file_path = $database_file_path_prefix.$orig_filename.".nii.gz";
							$new_job->expected_output = $orig_filename.$database_file_output_suffix;
							$file_uploaded = true;
					  	if (file_exists($upload_location . $_FILES["uploadedfile$mid"]["name"][$cnt])) {
								...
								if ( $upload_fileexist === "2" || $upload_fileexist === "1" ) {
									// backup option
									$time_recorded = microtime(true);
									$new_filename = $orig_filename . $time_recorded . ".nii.gz";
									//DATABASE
									$new_job->file_path = $database_file_path_prefix.$new_filename;
									$new_job->expected_output = $orig_filename.$time_recorded.$database_file_output_suffix;
						...
						if (strpos($rowdata, $upload_formfieldsdiv) >= 0) {
							...
							if (strlen($rowArray[0]) > 0) {
								...
								for ($cnt2 = 0; $cnt2<count($formfieldsval); $cnt2++) {
									...
									//FOR DATABASE: FILL IN AGE AND GENDER FIELDS
									if ($ffArray[1] != '')
										$new_job_age = $ffArray[1];
									if ($ffArray[2] != '')
										$new_job_gender = $ffArray[2];
...
//echo "valfile=$valfile<br/>";
			...
			if ($file_uploaded == true) {
				$new_job->age = $new_job_age;
				$new_job->gender = $new_job_gender;
				$result = JFactory::getDbo()->insertObject('r17ie_user_jobs', $new_job);
				//echo $result;
			}
